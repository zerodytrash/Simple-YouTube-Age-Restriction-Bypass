...
(function() {
    ...
    var useBypassEverywhere = false; // This option can be enabled to block video ads (Adblock), but this can cause negative side effects~
    ...
    function inspectJsonData(parsedData) {
        try {
            ...
            if(parsedData.playerResponse && parsedData.playerResponse.playabilityStatus && parsedData.playerResponse.videoDetails) {
    ...
    function getUnlockedPlayerResponse(videoId) {
        ...
        xmlhttp.open("GET", "/get_video_info?video_id=" + videoId, false); // Synchronous!!!
        xmlhttp.send(null);
