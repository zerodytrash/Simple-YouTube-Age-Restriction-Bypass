...
const unlockStrategies = [
    ...
    // Source: https://github.com/yt-dlp/yt-dlp/issues/574#issuecomment-887171136
    {
        ...
        requireAuth: false,
        fn: (videoId) =>
            innertube.getPlayer(videoId, { clientScreen: 'EMBED' }, false),
    },
...
export function unlockPlayerResponse(playerResponse) {
    const videoId =
        ...
        innertube.getYtcfgValue('PLAYER_VARS').video_id;
    const reason =
        playerResponse.playabilityStatus?.status ||
        playerResponse.previewPlayabilityStatus?.status;
    ...
    if (unlockedPlayerResponse.playabilityStatus?.status !== 'OK') {
        ...
        throw new Error(
            `Player Unlock Failed, playabilityStatus: ${unlockedPlayerResponse.playabilityStatus?.status}`,
        );
    }
    ...
    if (
        ...
        unlockedPlayerResponse.streamingData?.adaptiveFormats
    ) {
        const cipherText =
        ...
        lastProxiedGoogleVideoUrlParams = videoUrl
            ? new URLSearchParams(new URL(videoUrl).search)
            : null;
    }
...
export function unlockNextResponse(originalNextResponse) {
    ...
    const {
        ...
        playlistId,
        index: playlistIndex,
    } = originalNextResponse.currentVideoEndpoint.watchEndpoint;
    const unlockedNextResponse = innertube.getNext(
        videoId,
        { clientScreen: 'EMBED' },
        playlistId,
        playlistIndex,
...
function mergeNextResponse(originalNextResponse, unlockedNextResponse) {
    ...
    const unlockedWatchNextFeed =
        unlockedNextResponse.contents?.singleColumnWatchNextResults?.results?.results?.contents?.find(
