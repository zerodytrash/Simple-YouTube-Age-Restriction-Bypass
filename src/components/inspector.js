...
export function isAgeRestricted(playabilityStatus) {
    ...
    if (Config.UNLOCKABLE_PLAYER_STATES.includes(playabilityStatus.status))
        return true;
...
export function isWatchNextObject(parsedData) {
    if (
        !parsedData?.contents ||
        !parsedData?.currentVideoEndpoint?.watchEndpoint?.videoId
    )
        return false;
    return (
...
export function isWatchNextSidebarEmpty(parsedData) {
    if (isDesktop) {
        // WEB response layout
        const result =
            parsedData.contents?.twoColumnWatchNextResults?.secondaryResults
    ...
    const content =
        parsedData.contents?.singleColumnWatchNextResults?.results?.results
            ?.contents;
    const result = content?.find(
        (e) => e.itemSectionRenderer?.targetId === 'watch-next-feed',
    )?.itemSectionRenderer;
...
export function isGoogleVideoUnlockRequired(
    googleVideoUrl,
